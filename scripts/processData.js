...
try {
  if (fs.existsSync(backupC1File)) {
    ...
    if (rows && rows.length > 0) {
      ...
      // Integración dinámica de los datos del archivo local de Cuenca E17 si es necesario
      // En este caso, C1 E17 R tiene información de participantes individuales de Lima o Cuenca.
      // Sumamos la cantidad de filas reales como asistencia verificada
  ...
  capReports.forEach(reportFile => {
    ...
    if (csvData && csvData.length > 2) {
      // Determinar qué sede es según el nombre del archivo
      let targetSede = 'CDMX';
      ...
      // Parseo quirúrgico según especificación FASE 1
      // Fila 1: Cabeceras de columnas (A: Métricas, B: E1, C: E2...)
      const headers = csvData[0];
      const equipos = headers.slice(1).filter(h => h !== '');

      // Fila 2/3: Fecha o Entrenador, Fila 3/4: Entrenador o Fecha
